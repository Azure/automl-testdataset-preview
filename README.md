...
![Models Tab](ModelTest_ModelsTab.png)
6. On the `model detail` page, navigate to the `Test results` tab. It shows the model test run(s). Click on a run in the list to view the test run details.
![Model Test Results Tab](ModelTest_TestResultsTab.png)
7. On the `model test run detail` page, click on the Output datasets to explore the predictions dataset.
Note: Work is in progress to easily download the predictions dataset file.
![Model Test Run Details](ModelTest_TestRunDetail.png)
## Limitations
