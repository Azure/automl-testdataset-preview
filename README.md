...
1. Browse to Home -> Automated ML. Click on New Automated ML run button and follow the steps to create a new AutoML run.
![New Automated ML Run](screenshots/ModelTest_NewAutomatedRun.png)
...
4. Click `Finish` button. After the run completes, navigate to the `Models` tab of the run and click on the `best model`.
![Models Tab](screenshots/ModelTest_ModelsTab.png)

6. On the `model details` page, navigate to the `Test results` tab. It shows the model test run(s). Click on a run in the list to view the test run details.
...
1. Navigate to the `Models` tab of the run and click on the `model` link for which a `model test` run has to be created.
![Models Tab](screenshots/ModelTest_ModelsTab.png)

2. On the `model details` page, click on the `Test model` button to open the `Test model` panel.
![Test Model Button](screenshots/ModelTest_Rundetails_TestModel.png)

3. On the `Test model` panel, select `compute cluster` and a `test dataset` and click on the `Test` button. The schema of the test dataset should match the training dataset (The Target column is optional).
![Test Model Panel](screenshots/ModelTest_TestModelPanel.png)

4. On successful creation of `model test run` details pages will be 
## Limitations
